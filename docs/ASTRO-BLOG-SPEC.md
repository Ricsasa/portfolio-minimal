...
1. **Create the folder.** Under `src/content/blog/`, add a new directory
   named after the slug you want (kebab-case, no dates, e.g.
   `shipping-project-x`). For a multi-language site, nest it under the locale
   folder first: `src/content/blog/<locale>/<slug>/`.
2. **Add `index.mdx`** inside that folder with frontmatter matching the
   schema in `src/content.config.ts`:

   ```mdx
   ---
   title: "Post title"
   description: "One or two sentences — used in the list page and as the meta description / OG description."
   pubDate: 2026-08-10
   tags: ["tag-one", "tag-two"]
   cover: "./cover.jpg"       # optional, path relative to this file
   coverAlt: "Alt text for the cover image"
   draft: false               # set true to keep it out of the list/routes until ready
   ---
   ```

   (Add `lang: "es-MX"` etc. instead of nesting by folder if the repo uses a
   flat directory + frontmatter-field convention rather than folder-per-locale.)

3. **Write the body** in Markdown/MDX below the frontmatter:
   - Plain `![alt](./screenshot.jpg)` for images colocated in the same folder
     — they're auto-optimized, no `<Image>` import needed.
   - Fenced code blocks (```ts ... ```) get Shiki highlighting automatically.
   - Use the custom rich components where the story needs more than plain
     markdown, e.g. `<Figure src={...}>caption</Figure>`,
     `<Callout title="Note">...</Callout>`, or a `> quote` (auto-mapped to
     `Callout`).
4. **Drop in any images** referenced by relative path directly into the same
   post folder (`cover.jpg`, `screenshot-1.jpg`, ...).
5. **Regenerate types** if this is the first post since a schema change:
   `npx astro sync` (safe, not a build — just refreshes `astro:content` types).
6. **Preview it**: `astro dev --background`, then visit `/blog` to confirm
   the card appears (sorted by `pubDate`, newest first) and `/blog/<slug>` to
   confirm the post renders — code block highlighted, images optimized, any
   `Callout`/`Figure` used showing correctly. Stop the server when done
   (`astro dev stop`).
7. **Publish**: leave `draft: false` (or remove the field, it defaults to
   `false`) and commit. Setting `draft: true` at any point pulls the post out
   of both the list and the route entirely — it won't build, not just hide
   from nav.
